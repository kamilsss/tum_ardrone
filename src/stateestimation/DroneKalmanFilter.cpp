	...
	while(true)
	{
		if(consume)
		{
			if(node->logfileFilter != NULL)
			{
				...
				(*(node->logfileFilter)) <<  -1 << " " << predictdUpToTimestamp << " "  // A 0 - 1
					<< x.state[0] << " " << y.state[0] << " " << z.state[0] << " "  	// 2 - 4
					<< roll.state << " " << pitch.state << " " << yaw.state[0] << " "   // 5 - 7
					<< x.state[1] << " " << y.state[1] << " " << z.state[1] << " " << yaw.state[1] << " " // 8 - 11
		...
		if(consume)
		{
			...
			{
				pthread_mutex_lock(&(node->logFilter_CS));
				(*(node->logfileFilter)) << predictdUpToTimestamp << " "  // A 0
					<< 0 << " " << 0 << " " << 0 << " " << 0 << " " << 0 << " " << 0 << " "  // B - G 1,2,3,4,5,6
					<< controlIterator->twist.linear.y << " " << controlIterator->twist.linear.x << " " << controlIterator->twist.linear.z << " " // H - I - J 7, 8, 9
					<< controlIterator->twist.angular.z << " "  // K 3 10
					<< (observedRPY ? rpyIterator->rotX : -1) << " " // L 11
					<< (observedRPY ? rpyIterator->rotY : -1) << " " // M 12
					<< (observedRPY ? lastdYaw : -1) << " "			 // N 13
					<< (observedXYZ ? xyzIterator->vx : -1) << " "	 // O 14
					<< (observedXYZ ? xyzIterator->vy : -1) << " "	 // P 15
					<< (observedXYZ ? lastdZ : -1) << " "			 // Q 16
					<< x.state[0] << " " << y.state[0] << " " << z.state[0] << " "  	// R - S - T 17 19
					<< roll.state << " " << pitch.state << " " << yaw.state[0] << " "   // U - V - W 29 22
					<< x.state[1] << " " << y.state[1] << " " << z.state[1] << " " << yaw.state[1] << " " // X - Y - Z - AA 23, 24 , 25, 26
					<< lastVXGain << " " << lastVYGain << " " << "\n";	// AB - AC 27, 28
				pthread_mutex_unlock(&(node->logFilter_CS));
			}
		}
